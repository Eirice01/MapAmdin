     ...
     :close-on-click-modal='false'>
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm" :disabled="isView">
        ...
        <el-form-item label="最高速度" prop="maxSpeed">
          <el-input v-model="ruleForm.maxSpeed" style="width: 50%" placeholder="请输入最高速度"></el-input>  km/h（0表示无限制）
        </el-form-item>
        <el-form-item label="最低速度" prop="minSpeed">
          <el-input v-model="ruleForm.minSpeed" style="width: 50%" placeholder="请输入最低速度"></el-input>  km/h（0表示无限制）
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer" v-show="!isView">
        ...
        <el-button type="primary" v-if="title==='添加区域限速报警设置'" @click="confirmAdd">确 定</el-button>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        minSpeed: [
          { required: true, message: '最低速度不能为空', trigger: 'blur' },
          { max: 20, message: '长度不能超过20个字符', trigger: 'blur' }
        ],
        remindingInterval: [
  ...
  methods: {
    ...
    handleUpdate(id) {
      ...
      fetchInfo(id).then(res => {
        console.log(res)
        if (res.code === 0) {
          res.data.minSpeed = String(res.data.minSpeed)
          res.data.maxSpeed = String(res.data.maxSpeed)
          this.ruleForm = res.data
        }
      })
    },
